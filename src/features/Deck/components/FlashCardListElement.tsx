...
export const FlashCardListElement: React.FC<FlasCardListElementProps> = ({ flashCard, foreignLanguageName, onEdit }) => {
    ...
    function renderDeleteConfirmation(){
        return <DeleteCardConfirmationDialog isOpen={isDeleteConfirmationOpen} onClose={handleDeleteConfirmationClose} flashCard={flashCard} foreignLanguageName={foreignLanguageName}/>;
    }
            ...
            <Card variant='elevated' shadow='md' flex={1}>
                <CardBody display='flex' flexDirection='row' alignItems='center' justifyContent='space-between'>
                    <Box>
                        ...
                        <Text fontSize='t2' userSelect='text'>{flashCard.translatedWord}</Text>
                    </Box>
